...
def collect_embeddings(
    ...
    # import pdb; pdb.set_trace()
    # data = datasets["dev" if args.stage == "decs" else "train"]
    if args.stage == "decs":
        data = datasets.tensors[0][: 3000]
    else: 
        data = datasets.tensors[0][3000 :]
    # data = datasets
    N_seqs = len(data)
    ...
    extract = lambda i: pt.tensor(data[i], dtype=pt.int32)
    # extract = data
    N_batches = int(math.ceil(N_seqs / args.batch_size))

    import pdb; pdb.set_trace()
    # if model_name == 'adept-bert-base-uncased' or model_name == 'bert-base-uncased':
    #     C, D, model = strip_model_bert(model, args.device)
    # else:
