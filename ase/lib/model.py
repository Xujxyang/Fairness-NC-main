...
def strip_model_bert_o(
    ...
    """
    # import pdb; pdb.set_trace()
    # C, D = model.cls.predictions.decoder.out_features, model.cls.predictions.decoder.in_features
    # W = list(model.cls.predictions.decoder.parameters())[0].detach()
    C, D = 30522, 768
    del model.bert.pooler
    model.bert.pooler = Identity()
    del model.dropout
    model.dropout = Identity()
    del model.classifier
    model.classifier = Identity()
    # del model.prefix_encoder
    # model.prefix_encoder = Identity()

    return C, D, model.to(device)

def strip_model_bert(
    model: AutoCLM, device: str = "cpu"
) -> Tuple[int, int, AutoCLM, Tensor]:
    """Split the model into the embedding layers and the classifier.
    model: CausalLM.
    ...
    """
    # import pdb; pdb.set_trace()
    C, D = model.bert_mlm.cls.predictions.decoder.out_features, model.bert_mlm.cls.predictions.decoder.in_features
    W = list(model.bert_mlm.cls.predictions.decoder.parameters())[0].detach()
    del model.bert_mlm.cls.predictions.decoder
    model.bert_mlm.cls.predictions.decoder = Identity()
    return C, D, model.to(device), W.to(device)
...
def get_classifier_weights(model_name: str, args: Namespace) -> Optional[Tensor]:
    ...
    # ckpt_path = select_ckpt(model_path, ckpt_idx)
    # TS
    # model_path = 'TinyStories-12x1024_10L'
    # _, _, ckpt_idx = split_parts(model_name)
    # ckpt_path = select_ckpt(model_path, ckpt_idx)
    # Bert
