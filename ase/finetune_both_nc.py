...
def train(data, args):
    ...
    writer = SummaryWriter('./logs_vallina_nc3/all-100')
    ...
    dataset = convert_features_to_dataset(features)  # all_inpt_ids: dataset.tensors[0] 
    ...
    #     single=False,
    #     stage="decs",
    #     stats_dir="bert-ase_collect",
    #     verbose=False
    # )
    # collect_embeddings(coll_args, model, dataset)
    ...
    indices = []
    file_path = '/opt/data/private/NC/BERT-ASE/ase_word_indices_gender.json'
...
#         print("epoch {}".format(str(epoch_i+1)))
        ...
        for step, batch in enumerate(batch_iterator):
            ...
            mlm_loss, orthogonal_loss, output = model(input_ids=b_input_ids, attention_mask=b_input_mask,
            ...
            # import pdb; pdb.set_trace()
            global_step = epoch_i * len(batch_iterator) + step
            # NC3
            X, Y = process_batch(output.hidden_states[12].detach(), b_input_ids, b_input_mask, torch.cuda.current_device())
            stats.collect_means(X.detach(), Y.detach(), b_input_ids.size(0))
            W = list(model.bert_mlm.cls.predictions.decoder.parameters())[0]
...
def main():
    ...
    parser.add_argument("--save_path", default="./bert_debias_vallina_nc3_all_100", type=str, help="Path for saving results")
