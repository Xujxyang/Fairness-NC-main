...
def cl_forward(
    ...
    if token_type_ids is not None:  # e.g. for roberta
        ...
        # import pdb; pdb.set_trace()
        # NC batch
        X_orig_z0, Y_orig_z0 = process_batch(orig_z0_outputs.last_hidden_state.detach(), orig_z0_input_ids, orig_z0_mask, dist.get_rank())
        X_orig_z1, Y_orig_z1 = process_batch(orig_z1_outputs.last_hidden_state.detach(), orig_z1_input_ids, orig_z1_mask, dist.get_rank())
        X_aug_z0, Y_aug_z0 = process_batch(aug_z0_outputs.last_hidden_state.detach(), aug_z0_input_ids, aug_z0_mask, dist.get_rank())
        X_aug_z1, Y_aug_z1 = process_batch(aug_z1_outputs.last_hidden_state.detach(), aug_z1_input_ids, aug_z1_mask, dist.get_rank())
    ...
    #     # Dummy vectors for allgather
    #     X_orig_z0_list = [torch.zeros_like(X_orig_z0) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=X_orig_z0_list, tensor=X_orig_z0.contiguous())
    #     X_orig_z0_list[dist.get_rank()] = X_orig_z0
    #     X_orig_z0 = torch.cat(X_orig_z0_list, 0)
    #     print(X_orig_z0.shape)

    #     Y_orig_z0_list = [torch.zeros_like(Y_orig_z0) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=Y_orig_z0_list, tensor=Y_orig_z0.contiguous())
    #     Y_orig_z0_list[dist.get_rank()] = Y_orig_z0
    #     Y_orig_z0 = torch.cat(Y_orig_z0_list, 0)
    #     print(Y_orig_z0.shape)

    #     X_orig_z1_list = [torch.zeros_like(X_orig_z1) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=X_orig_z1_list, tensor=X_orig_z1.contiguous())
    #     X_orig_z1_list[dist.get_rank()] = X_orig_z1
    #     X_orig_z1 = torch.cat(X_orig_z1_list, 0)
    #     print(X_orig_z1.shape)

    #     Y_orig_z1_list = [torch.zeros_like(Y_orig_z1) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=Y_orig_z1_list, tensor=Y_orig_z1.contiguous())
    #     Y_orig_z1_list[dist.get_rank()] = Y_orig_z1
    #     Y_orig_z1 = torch.cat(Y_orig_z1_list, 0)
    #     print(Y_orig_z1.shape)

    #     X_aug_z0_list = [torch.zeros_like(X_aug_z0) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=X_aug_z0_list, tensor=X_aug_z0.contiguous())
    #     X_aug_z0_list[dist.get_rank()] = X_aug_z0
    #     X_aug_z0 = torch.cat(X_aug_z0_list, 0)
    #     print(X_aug_z0.shape)

    #     Y_aug_z0_list = [torch.zeros_like(Y_aug_z0) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=Y_aug_z0_list, tensor=Y_aug_z0.contiguous())
    #     Y_aug_z0_list[dist.get_rank()] = Y_aug_z0
    #     Y_aug_z0 = torch.cat(Y_aug_z0_list, 0)
    #     print(Y_aug_z0.shape)
        
    #     X_aug_z1_list = [torch.zeros_like(X_aug_z1) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=X_aug_z1_list, tensor=X_aug_z1.contiguous())
    #     X_aug_z1_list[dist.get_rank()] = X_aug_z1
    #     X_aug_z1 = torch.cat(X_aug_z1_list, 0)
    #     print(X_aug_z1.shape)

    #     Y_aug_z1_list = [torch.zeros_like(Y_aug_z1) for _ in range(dist.get_world_size())]
    #     dist.all_gather(tensor_list=Y_aug_z1_list, tensor=Y_aug_z1.contiguous())
    #     Y_aug_z1_list[dist.get_rank()] = Y_aug_z1
    #     Y_aug_z1 = torch.cat(Y_aug_z1_list, 0)
    #     print(Y_aug_z1.shape)
    ...
    if cls.model_args.a1_loss: #only a1 used
    ...
    # collect_means
    stats.collect_means(X_orig_z0.detach(), Y_orig_z0.detach(), orig_z0_input_ids.size(0))
    stats.collect_means(X_orig_z1.detach(), Y_orig_z1.detach(), orig_z1_input_ids.size(0))
    stats.collect_means(X_aug_z0.detach(), Y_aug_z0.detach(), aug_z0_input_ids.size(0))
    stats.collect_means(X_aug_z1.detach(), Y_aug_z1.detach(), aug_z1_input_ids.size(0))
    
    # NC3
    W = list(cls.lm_head.decoder.parameters())[0]
    ...
    # NC1
    # import pdb; pdb.set_trace()
    # stats.collect_vars(X_orig_z0.detach(), Y_orig_z0.detach(), orig_z0_input_ids.size(0) * dist.get_world_size())
    # stats.collect_vars(X_orig_z1.detach(), Y_orig_z1.detach(), orig_z1_input_ids.size(0) * dist.get_world_size())
    # stats.collect_vars(X_aug_z0.detach(), Y_aug_z0.detach(), aug_z0_input_ids.size(0) * dist.get_world_size())
    # stats.collect_vars(X_aug_z1.detach(), Y_aug_z1.detach(), aug_z1_input_ids.size(0) * dist.get_world_size())
    
    # indices_have = [idx for idx in indices if stats.counts[idx] != 0]
    # CDNVs = stats.compute_vars(indices_have, 1024)
    # if CDNVs is not None:
    #     mean = triu_mean(CDNVs)
    #     std = triu_std(CDNVs, mean)
    # loss_nc3 = std
    # total_loss
    # import pdb; pdb.set_trace()
    name_loss_nc3 = "loss_nc3_" + str(dist.get_rank())
    writer.add_scalar(name_loss_nc3, loss_nc3, step)
    name_loss_ori = "name_loss_ori_" + str(dist.get_rank())
    writer.add_scalar(name_loss_ori, loss, step)

    loss = loss + 10 * loss_nc3

    name_loss_total = "loss_total_" + str(dist.get_rank())
    writer.add_scalar(name_loss_total, loss, step)
...
class BertncForMabel(BertPreTrainedModel):
