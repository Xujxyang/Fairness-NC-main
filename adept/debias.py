...
def create_dataset(data, dataset):
    d = dict()
    # import pdb; pdb.set_trace()
    for key in data['example'].keys():  #13688  57252
...
def create_nc_dataset(data, dataset):
    combined_examples = []
    # import pdb; pdb.set_trace()
    for key in data['example'].keys():
        combined_examples.extend(data['example'][key])
    # for key in data['example'].keys():
    #     if key not in data['label']:
    #         d = dataset(data['example'][key], None)
    #     else:
    #         d = dataset(data['example'][key], data['label'][key])

    return combined_examples

def load_and_cache_examples(data, args, tokenizer):
    train_dataset = create_dataset(data['train'], LineByLineTextDataset) #create_nc_dataset
...
def train(args, data, datasets, model: PreTrainedModel, original_model, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    writer = SummaryWriter('./logs/epoch-5-gender1')
    ...
    def get_hiddens_of_model(input, input_attention_mask):
        ...
        if args.model_type == 'roberta':
            ...
            if args.algorithm == 'ADEPT':
                ...
                # hiddens = model.module.bert(input).hidden_states
    ...
    def forward(attributes_hiddens, dataloaders, key, stats=None, indices=None, writer=None, step=None):
        ...
        if stats != None:
            X, Y = process_batch(all_layer_hiddens[12].detach(), inputs, inputs_attention_mask, torch.cuda.current_device())
        ...
        if args.loss_target == 'sentence' or labels is None:
            ...
            if labels.size(1) > 1:
                ...
                # import pdb; pdb.set_trace()
                onehot = torch.eye(target_layer_hiddens.size(1), device=labels.device)[labels].view(target_layer_hiddens.size(0), -1, 1, 1)
            onehot = onehot.to(args.device)
        ...
        if stats != None:
            # import pdb; pdb.set_trace()
            stats.collect_means(X.detach(), Y.detach(), inputs.size(0))
            # single do not have module
    ...
    indices = []
    file_path = '/opt/data/private/NC/ADEPT/adept_word_indices_gender_1.json'
    with open(file_path, 'r', encoding='utf-8') as file:
        data_json = json.load(file)
    # for index in data_json['bias_classification']['gender'].values():
    ...
    # from torch.utils.tensorboard import SummaryWriter
    # writer = SummaryWriter('./logs/all-0.05')
    # import pdb; pdb.set_trace()
    epoch_global = -1
    for _ in train_iterator:
        ...
        for step, key in enumerate(epoch_iterator):
            ...
            # import pdb; pdb.set_trace()
            step_global = epoch_global * len(epoch_iterator) + step
            loss = forward(attributes_hiddens, train_dataloaders, key, stats, indices, writer, step_global)
...
def main():
    ...
    attributes_examples = data['attributes_examples'] #[2, 14588]
    attributes_labels = data['attributes_labels']
    neutral_examples = data['neutral_examples']

    #NC!!!
    #NC no random shuffule in split data
    ...
    # different data type(nc)
    datasets = load_and_cache_examples(splited_data, args, tokenizer) # datasets['train']['attribute0'][0]
    # import pdb; pdb.set_trace()
    # do collect
    ...
    #     single=False,
    #     stage="decs",
    #     stats_dir="adept_collect_religion_1",
    #     verbose=False
    # )
    # collect_embeddings(coll_args, model, datasets)
