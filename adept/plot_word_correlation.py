...
def calculate(args, data, datasets, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def get_logits_of_model(input, input_attention_mask):
        if hasattr(torch.cuda, 'empty_cache'):
            torch.cuda.empty_cache()
        ...
        with torch.no_grad():
            if args.model_type == 'roberta':
                if args.tuning_type == 'prompt_tuning':
                    hiddens = model(input, input_attention_mask).logits
                elif args.tuning_type == 'finetuning':
                    hiddens = model(input).logits
            elif args.model_type == 'bert':
                if args.tuning_type == 'prompt_tuning':
                    hiddens = model(input, input_attention_mask).logits
                elif args.tuning_type == 'finetuning':
    ...
    for word in tqdm(word_list, desc="word list"):
        word_source = ["herself", "himself", "mother", "father"] #"brother", "sister"
        ...
        hiddens = get_logits_of_model(inputs, inputs_attention_mask)
        if labels.size(1) > 1:
            ...
            onehot = onehot.view(hiddens.size(0), -1, 1) #(hiddens.size(0), -1, 1, 1)
        else:
            onehot = torch.eye(hiddens.size(1))[labels].view(hiddens.size(0), -1, 1)
